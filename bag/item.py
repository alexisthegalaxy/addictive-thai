...
class Item(object):
    def __init__(self, name_id, name=None, description=None, price=10, amount=1, durability=None, is_spell=False):
        self.name_id = name_id  # for example, 'lomsak_apple'
        if not name:
            item = list(
                CURSOR.execute(
                    f"""
                    SELECT name, description, price, is_spell
                    FROM items
                    WHERE id = '{name_id}'
                    """
                )
